...
def calculate_next_weekday(weekday):
    ...
    next_weekday_this_time = now + datetime.timedelta(days=number_days_until_next_weekday(now, weekday))
    next_weekday_this_hour = next_weekday_this_time.replace(minute=0, second=0, microsecond=0)
